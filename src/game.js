...
<!-- Toca Prin-Wolf -->
  ...
  // buildCartoonCat removed from runtime — see docs/saved-cats.js to restore
      ...
      catStartPositions.forEach((startPos, i) => {
        BABYLON.SceneLoader.ImportMeshAsync('', 'sprites/', 'kotek.glb', scene).then(r => {
          ...
          scene.onBeforeRenderObservable.add(() => {
            bt += 0.025;
            kr.position.y = (kr._baseY || 0) + Math.sin(bt) * 0.02;
          });

          // Roam AI — pick waypoint, walk, pause, repeat
          let tX = startPos[0], tZ = startPos[2];
          let pausing = false, pauseTimer = 0;
          let meowTimer = 2 + Math.random() * 6 + i * 1.5; // stagger meows

          function newWaypoint() {
            tX = startPos[0] + (Math.random() - 0.5) * 14;
            tZ = startPos[2] + (Math.random() - 0.5) * 14;
            tX = Math.max(-9, Math.min(9, tX));
            tZ = Math.max(-9, Math.min(9, tZ));
          }
          newWaypoint();
          ...
          scene.onBeforeRenderObservable.add(() => {
            ...
            const dx = tX - kr.position.x, dz = tZ - kr.position.z;
          ...
          console.log('[doom2] Kotek cat', i, 'spawned at', startPos[0], startPos[2]);
